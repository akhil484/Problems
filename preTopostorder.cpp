...
Node *constructTreeUtil(int pre[],int *pre_index,int key,int size,int min, int max)
{
    if(*pre_index>=size)
        return NULL;
    Node *root=NULL;
    if(key>min && key<max)
    {
        ...
        *pre_index = *pre_index+1;
        if(*pre_index<size)
        {
            root->left=constructTreeUtil(pre,pre_index,pre[*pre_index],size,min,key);
        }
        if(*pre_index<size)
        {
            root->right=constructTreeUtil(pre,pre_index,pre[*pre_index],size,key,max);
        }
    }
    return root;
}
Node* constructTree(int pre[], int size) {
    //code here
    int pre_index=0;
    return constructTreeUtil(pre,&pre_index,pre[0],size,INT_MIN,INT_MAX);
